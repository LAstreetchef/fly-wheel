...
import { startMetricsSync, setOrderStore as setMetricsOrderStore } from './server/jobs/syncMetrics.js';
import { startAutoBoostScheduler, setPool as setAutoBoostPool, initAutoBoostsTable } from './server/jobs/autoBoost.js';
...
// Inject order store into background jobs
setOrderStore(orderStore);
setMetricsOrderStore(orderStore);

// Inject pool into growth services (for referrals and auto-boost)
if (usePostgres) {
  const growthPool = new pg.Pool({
    connectionString: process.env.DATABASE_URL,
    ssl: process.env.NODE_ENV === 'production' ? { rejectUnauthorized: false } : false,
  });
...
const generateLimiter = rateLimit({
  ...
  max: 20, // 20 generations per hour per email+IP (prevents Claude abuse)
  keyGenerator: (req) => {
...
const checkoutLimiter = rateLimit({
  windowMs: 60 * 60 * 1000, // 1 hour
  max: 10, // 10 checkout attempts per hour per email+IP
  keyGenerator: (req) => {
...
app.get('/api/blogs/search', sanitizeQuery(['keywords'], 200), async (req, res) => {
  try {
...
// ============================================
// Free Preview (3/hour per IP)
// ============================================
const previewLimiter = rateLimit({
  windowMs: 60 * 60 * 1000,
  max: 3,
  message: { error: 'Free preview limit reached. Purchase a boost or try again later.' },
  standardHeaders: true,
  legacyHeaders: false,
});

app.post('/api/preview', sanitizeBody(['product', 'keywords'], 500), previewLimiter, async (req, res) => {
  try {
    const { product, keywords } = req.body;
    if (!product || !keywords) {
      return res.status(400).json({ error: 'product and keywords required' });
    }

    // Step 1: Find blogs (uses cache)
    const blogs = await searchBlogs(keywords, 3);
    if (!blogs.length) {
      return res.json({ blogs: [], content: null, message: 'No relevant blogs found. Try different keywords.' });
    }

    // Step 2: Generate content for the top blog (uses cache)
    const topBlog = blogs[0];
    const content = await generateBoostContent(
      { name: product, keywords },
      { title: topBlog.title, url: topBlog.url, snippet: topBlog.snippet }
    );

    // Step 3: Return preview (no posting, no payment)
    res.json({
      blogs,
      content,
      selectedBlog: topBlog,
      message: 'Preview generated! Pay $1.99 to post this tweet.',
    });
  } catch (error) {
...
// Reddit Integration
// ============================================
app.post('/api/reddit/search', sanitizeBody(['keywords'], 200), generateLimiter, async (req, res) => {
  try {
    const keywords = req.query.keywords || req.body.keywords;
    if (!keywords) return res.status(400).json({ error: 'Keywords required' });
    const threads = await searchRedditThreads(keywords);
    res.json({ threads });
  } catch (err) {
...
app.post('/api/reddit/generate', sanitizeBody(['product', 'keywords'], 500), generateLimiter, async (req, res) => {
  try {
    const { product, keywords, threadTitle, threadUrl } = req.body;
    if (!product || !threadTitle) {
      return res.status(400).json({ error: 'product and threadTitle required' });
    }
    const comment = await generateRedditComment({ product, keywords, threadTitle, threadUrl });
    res.json({ comment, platform: 'reddit' });
  } catch (err) {
    console.error('Reddit generate error:', err.message);
...
app.get('/api/referral/:email', async (req, res) => {
  try {
    const code = await getOrCreateReferralCode(req.params.email);
    const stats = await getReferralStats(req.params.email);
    res.json({
      referralCode: code,
      referralUrl: `https://daufinder.com?ref=${code}`,
      stats,
    });
  } catch (err) {
...
app.post('/api/referral/redeem', sanitizeBody(['referralCode', 'email'], 200), async (req, res) => {
  try {
    const { referralCode, email } = req.body;
    if (!referralCode || !email) {
      return res.status(400).json({ error: 'referralCode and email required' });
    }
    const result = await processReferral(referralCode, email);
    if (!result.success) {
...
// ============================================
// Auto-Boost (Prime Feature)
// ============================================

// Create auto-boost subscription
app.post('/api/prime/auto-boost', sanitizeBody(['email', 'product', 'keywords', 'frequency'], 500), async (req, res) => {
  try {
    const { email, product, keywords, frequency } = req.body;

    // Verify Prime account
    const account = await primeStore.get(email);
    if (!account) {
      return res.status(403).json({ error: 'Prime account required' });
    }

    const validFreqs = ['daily', 'weekly'];
    const freq = validFreqs.includes(frequency) ? frequency : 'weekly';

    // Use the growth pool for auto_boosts table
    const growthPool = new pg.Pool({
      connectionString: process.env.DATABASE_URL,
      ssl: process.env.NODE_ENV === 'production' ? { rejectUnauthorized: false } : false,
    });

    const result = await growthPool.query(
      `INSERT INTO auto_boosts (email, product, keywords, frequency, next_run_at)
       VALUES ($1, $2, $3, $4, NOW() + INTERVAL '1 hour')
       RETURNING *`,
      [email.toLowerCase(), product, keywords, freq]
    );

    await growthPool.end();
    res.json({ autoBoost: result.rows[0], message: `Auto-boost set: ${freq}` });
  } catch (err) {
    console.error('Auto-boost create error:', err.message);
    res.status(500).json({ error: 'Failed to create auto-boost' });
  }
});

// List auto-boosts for a user
app.get('/api/prime/auto-boosts/:email', async (req, res) => {
  try {
    const growthPool = new pg.Pool({
      connectionString: process.env.DATABASE_URL,
      ssl: process.env.NODE_ENV === 'production' ? { rejectUnauthorized: false } : false,
    ...
    const result = await growthPool.query(
      'SELECT * FROM auto_boosts WHERE LOWER(email) = LOWER($1) ORDER BY created_at DESC',
      [req.params.email]
    );

    await growthPool.end();
    res.json({ autoBoosts: result.rows });
  } catch (err) {
    console.error('Auto-boost list error:', err.message);
    res.status(500).json({ error: 'Failed to list auto-boosts' });
  }
});

// Cancel auto-boost
app.delete('/api/prime/auto-boost/:id', async (req, res) => {
  try {
    const growthPool = new pg.Pool({
      connectionString: process.env.DATABASE_URL,
      ssl: process.env.NODE_ENV === 'production' ? { rejectUnauthorized: false } : false,
    ...
    await growthPool.query('UPDATE auto_boosts SET active = false WHERE id = $1', [req.params.id]);
    await growthPool.end();
    res.json({ message: 'Auto-boost cancelled' });
  } catch (err) {
    console.error('Auto-boost cancel error:', err.message);
    res.status(500).json({ error: 'Failed to cancel auto-boost' });
  }
});
...
app.post('/webhook', async (req, res) => {
  ...
  if (event.type === 'checkout.session.completed') {
    ...
    // Regular one-time payment — queue for async processing
    ...
    if (order) {
      ...
      try {
        ...
        // Queue the heavy lifting (blog search if needed + AI + tweet)
        queueBoost({
          ...
          content,
          source: 'paid',
          priority: 1, // Paid boosts get priority
        });
...
app.listen(PORT, () => {
  ...
  startMetricsSync();
  startAutoBoostScheduler();
  console.log('📊 Background jobs started (metrics sync + auto-boost scheduler)');
});
